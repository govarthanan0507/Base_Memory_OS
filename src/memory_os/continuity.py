...
def _entity(store: MemoryStore, entity_id: str):
    ...
    for sql,kind in queries:
        ...
        if row is not None: return kind,dict(row)
    return None
...
def render_reentry_brief(store: MemoryStore, conversation_id: str, limit: int = 50) -> str:
    p=conversation_context(store,conversation_id,limit); c=p["conversation"]; lines=[f"# Re-entry: {c['title']}","",f"Source: {c['source']}","","## Known related work"]
...
def render_project_reentry_brief(store: MemoryStore, project_id: str, limit: int = 50) -> str:
    p=project_context(store,project_id,limit); x=p["project"]; lines=[f"# Project Re-entry: {x['name']}","",f"Status: {x['status']}",f"Root: {x['root']}","","## Artifacts"]
    lines += [f"- {a['name']} ({a.get('artifact_type','unknown')}) — {a.get('location','')}" for a in p["artifacts"]] or ["- No linked artifacts."]
    lines += ["","## Conversations"]+([f"- {c['title']} [{c['source']}] ({c.get('relation','related')})" for c in p["conversations"]] or ["- No linked conversations."])
    lines += ["","## Linked memories"]+([f"- [{m['memory_type']}] {m['content']}" for m in p["memories"]] or ["- No linked memories."])
    return "\n".join(lines)
