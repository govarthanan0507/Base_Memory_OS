...
def ensure_research_candidate_schema(store: MemoryStore) -> None:
    store.conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS research_candidate_evidence (
            ...
            metadata_json TEXT NOT NULL,
            UNIQUE(candidate_id, content_hash, evidence)
        );
        """
    )
    store.conn.commit()
...
def persist_research_candidates(
    ...
    for candidate in candidates:
        ...
        evidence_id = stable_id(
            "research-evidence", candidate_id, candidate.content_hash, candidate.evidence
        )
        store.conn.execute(
...
__all__ = [
    "PersistedResearchCandidate",
    "ensure_research_candidate_schema",
