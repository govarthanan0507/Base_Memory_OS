...
def _messages_from_json(data: dict[str, Any]) -> tuple[Message, ...]:
    ...
    if not isinstance(raw, list):
        raise ValueError("JSON conversation requires a messages list")
    messages = []
    for i, item in enumerate(raw, 1):
        ...
        if not role or not content:
            raise ValueError(f"message {i} requires role and content")
        messages.append(Message(role=role, content=content, sequence=i,
...
def conversation_from_json(data: dict[str, Any], source_location: str | None = None) -> Conversation:
    return Conversation(
        ...
        messages=_messages_from_json(data),
    )
...
def conversation_from_markdown(text: str, source: str = "markdown", title: str = "Untitled conversation",
    ...
    def flush() -> None:
        ...
        if role and content:
            sequence += 1
            messages.append(Message(role=role, content=content, sequence=sequence))
        buffer = []
    ...
    flush()
    if not messages:
        raise ValueError("Markdown conversation requires role markers such as 'user:' and 'assistant:'")
...
def import_markdown(store: MemoryStore, path: str | Path, *, source: str = "markdown", title: str | None = None) -> str:
    ...
    conversation = conversation_from_markdown(
        source_path.read_text(encoding="utf-8"), source=source,
        title=title or source_path.stem, source_location=str(source_path.resolve())
    )
