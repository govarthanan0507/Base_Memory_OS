...
PROJECT_MARKERS = {
    ...
    "Pipfile", "poetry.lock", "composer.json", "pom.xml", "build.gradle", "CMakeLists.txt",
    "README.md", ".git"
}
...
def likely_project_roots(root: Path) -> list[Path]:
    """Find project roots without changing anything on disk."""
    root = root.resolve()
    candidates: list[Path] = []
    for current, dirs, files in os.walk(root):
        dirs[:] = [d for d in dirs if d not in IGNORED_DIRS and not d.startswith(".")]
        marker_set = set(files) | ({".git"} if (Path(current) / ".git").is_dir() else set())
        if marker_set & PROJECT_MARKERS:
            candidates.append(Path(current))
    # Keep outer project roots and avoid reporting every nested file as a project.
...
def inspect_project(root: Path) -> Project:
    ...
    metadata = {
        "file_count": len(files), "code_file_count": len(code_files), "total_bytes": total_bytes,
        "markers": sorted(set(markers)), "discovery_version": "0.1.0"
    }
...
def scan_workspace(root: str | Path, store: MemoryStore) -> list[Project]:
    """Read-only scan. Registers projects and their file artifacts; never moves/deletes files."""
        ...
        for current, dirs, names in os.walk(project_root):
            ...
            for name in names:
                ...
                store.add_artifact(Artifact(
                    name=name,
                    artifact_type="code" if path.suffix.lower() in CODE_EXTENSIONS else "file",
                    location=str(path.resolve()),
                    content_hash=file_hash(path) if stat.st_size <= 20 * 1024 * 1024 else None,
                    modified_at=str(stat.st_mtime),
                    metadata={"project_root": str(project_root.resolve())},
                ))
    return projects


__all__ = ["inspect_project", "likely_project_roots", "scan_workspace"]
