...
_PATTERNS = (("decision", ("we decided", "i decided", "decided to", "let's use", "we'll use", "we will use"), .8), ("preference", ("i prefer", "i want", "i don't want", "i do not want", "prefer to", "you prefer"), .75), ("task", ("todo", "to do", "next step", "need to", "should build", "should add"), .7), ("unresolved", ("not sure", "need to figure out", "still need to", "what if", "how do we"), .6))
...
def extract_candidates(messages: Iterable[Message|dict[str,Any]], max_per_message:int=2, project_id:str|None=None)->list[MemoryCandidate]:
    ...
    out=[]
    for raw in messages:
        m=_normalize(raw); text=" ".join(m.content.strip().split())
        if not text: continue
        found=0
        for kind,patterns,confidence in _PATTERNS:
            if not any(p in text.lower() for p in patterns): continue
            adjusted=max(0.0,confidence-.2) if m.role!="user" and kind in _PERSONAL_TYPES else confidence
            sid=m.message_id or message_id("unbound",m.sequence,m.role,m.content,m.external_id)
            meta={"role":m.role,"observed_at":m.observed_at,"source_message_id":sid}
            if project_id: meta["project_id"]=project_id
            out.append(MemoryCandidate(kind,text,m.sequence,adjusted,metadata=meta)); found+=1
