...
**IMPL-04 — Research Memory v0.9 — persistent provenance-bound research registry**
...
The semantic layer identifies conservative candidates for referenced repositories, explicitly named tools, ideas and source topics. The new research registry gives those candidates durable identity using `kind + normalized value`, while retaining every distinct source/snapshot/evidence occurrence separately. Candidates can be explicitly accepted or rejected without automatically creating general durable memory.

## Verification status

Corrected-head CI run #224 (`35184592014`) passed all unit-test matrix jobs on Python 3.11, 3.12, 3.13 and 3.14. This verifies the provenance-chain regression and the persistent research registry tests.

Earlier failures remain recorded: run #200 exposed an invalid malformed-link fixture; corrected fixture run #211 exposed an uncaught `urljoin()` `ValueError`, which was fixed in `ba8204e920136c0aee975dec8ab9d5b07e41ae0c`. Run #223 then exposed a test API mismatch, fixed in `b86ee0505639701a9073768bb57788370be453c1`; run #224 verified that correction.
